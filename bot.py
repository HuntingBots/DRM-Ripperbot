...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = (
        ...
        "*🔧 Example:*\n"
        "`/mpdkey https://example.com/stream.mpd 87d54d841e2348b2969a66137522aedd:dc4df1cd96827c90fd05829ef8ed0a0f 720p`\n\n"
        "If you need help or your request fails, contact the bot admin."
    )
    # Send the welcome text
    try:
        await update.message.reply_text(welcome_text, parse_mode='Markdown')
    except Exception:
        # Fallback without parse mode if something fails
        await update.message.reply_text(welcome_text)

# Optional: a simple help command
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    help_text = (
        "Commands:\n"
        "/start - Show features\n"
        "/mpdkey <url> [key] [quality] - Download MPD with key\n"
        "/testkey <key> - Validate key format\n"
        "/extractkey <url> - Extract keys from URL\n"
    )
...
def main():
    ...
    application.add_handler(CommandHandler('mpdkey', mpdkey_command))
    application.add_handler(CommandHandler('testkey', test_key_command))
    application.add_handler(CommandHandler('extractkey', extract_key_command))

    # If inline callbacks are used elsewhere in your code, register the CallbackQueryHandler
    application.add_handler(CallbackQueryHandler(lambda u, c: None))  # placeholder no-op; replace as needed

    # You can also register other handlers here as your project requires
    # e.g. MessageHandler(filters.TEXT & ~filters.COMMAND, message_handler)
